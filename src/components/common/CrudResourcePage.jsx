...
export function CrudResourcePage({
  ...
  const remove = async (row) => {
    if (!window.confirm(`Delete ${row.name || row.id || 'this record'}?`)) return;
    try {
  ...
  return (
    <DashboardLayout>
      <div className="crud-page">
        <div className="crud-heading">
          ...
          <div className="crud-heading-actions">
            {actions}
            <button className="crud-btn crud-btn-primary" onClick={startAdd}><Plus size={17} />{addLabel}</button>
          </div>
        </div>

        {resolvedStats.length > 0 && (
          <div className="crud-stats">
            {resolvedStats.map((stat) => <div className="crud-stat" key={stat.label}><span>{stat.label}</span><strong>{stat.value}</strong></div>)}
          </div>
        )}
          ...
          {loading ? <div className="crud-empty">Loading…</div> : filtered.length === 0 ? <div className="crud-empty">No records found.</div> : (
            <>
                ...
                <table className="crud-table">
                  ...
                  <tbody>{filtered.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td><div className="crud-row-actions"><button onClick={() => startEdit(row)} aria-label="Edit"><Edit3 size={16} /></button><button className="danger" onClick={() => remove(row)} aria-label="Delete"><Trash2 size={16} /></button></div></td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
              ...
              <div className="crud-mobile-list">
                {filtered.map((row) => (
                  <article className="crud-mobile-card" key={row.id}>
                    <div className="crud-mobile-card-head"><strong>{formatCell(row[columns[0]?.key], columns[0]?.type)}</strong><div className="crud-row-actions"><button onClick={() => startEdit(row)}><Edit3 size={16} /></button><button className="danger" onClick={() => remove(row)}><Trash2 size={16} /></button></div></div>
      ...
      {open && (
        <div className="crud-modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setOpen(false)}>
          <form className="crud-modal" onSubmit={save}>
            <div className="crud-modal-head"><div><span>{editing ? 'Edit record' : 'Create record'}</span><h2>{title}</h2></div><button type="button" className="crud-icon-btn" onClick={() => setOpen(false)}><X size={19} /></button></div>
            <div className="crud-form-grid">
              {fields.map((field) => (
                <label className={field.full ? 'crud-field full' : 'crud-field'} key={field.name}>
                  <span>{field.label}{field.required ? ' *' : ''}</span>
                  {field.type === 'select' ? (
                    <select required={field.required} value={form[field.name]} onChange={(e) => setForm({ ...form, [field.name]: e.target.value })}>
                      <option value="">Select</option>{(field.options || []).map((option) => <option value={typeof option === 'string' ? option : option.value} key={typeof option === 'string' ? option : option.value}>{typeof option === 'string' ? option : option.label}</option>)}
                    </select>
                  ) : field.type === 'textarea' ? (
                    <textarea required={field.required} rows="4" value={form[field.name]} onChange={(e) => setForm({ ...form, [field.name]: e.target.value })} />
                  ) : field.type === 'checkbox' ? (
                    <input type="checkbox" checked={Boolean(form[field.name])} onChange={(e) => setForm({ ...form, [field.name]: e.target.checked })} />
                  ) : (
                    <input required={field.required} type={field.type === 'currency' ? 'number' : (field.type || 'text')} min={field.min} step={field.step} value={form[field.name]} onChange={(e) => setForm({ ...form, [field.name]: e.target.value })} />
                  )}
                </label>
              ))}
            </div>
